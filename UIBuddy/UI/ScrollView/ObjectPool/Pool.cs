...
public class Pool<T> : Pool where T : IPooledObject
{
    ...
    public Pool()
    {
        ...
        InactiveHolder.transform.parent = PanelManager.PoolHolder.transform;
    ...
    public void ReturnObject(T obj)
    {
        if (!borrowed.Contains(obj))
            Plugin.Log.LogWarning($"Returning an item to object pool ({typeof(T).Name}) but the item didn't exist in the borrowed list?");
        else
            borrowed.Remove(obj);
